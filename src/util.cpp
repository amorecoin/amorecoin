...
const char * const BITCOIN_CONF_FILENAME = "amorecoin.conf";
const char * const BITCOIN_PID_FILENAME = "amorecoin.pid";
...
#else
    const char* pszModule = "amorecoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Amorecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Amorecoin";
#else
    // Unix
    return pathRet / ".amorecoin";
#endif
#endif
}
